			...
			while (featureReader.NextByte())
			{
				...
				switch ((FeatureType)featureType)
				{
					...
					case FeatureType.Geometry:
						...
						if (feat.GeometryType == GeomType.XYZ)
						{
							// HACK: to workaround validation checking for GeometryCommands
							feat.GeometryCommands = new List<uint>();
							feat.XYZraw = featureReader.GetPackedSInt32();
						}
						else
						{
